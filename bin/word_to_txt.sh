...
# fi
#
function __word_to_txt() {
	local DOCFILES=$@
	if [ -n "${DOCFILES}" ]; then
		local FUNC=${FUNCNAME[0]} MSG="None" STATUS A
		declare -A config_word_to_txt=()
		__load_util_conf "$UTIL_WORD_TO_TXT_CFG" config_word_to_txt
		STATUS=$?
		if [ $STATUS -eq $SUCCESS ]; then
			...
			if [ $STATUS -eq $SUCCESS ]; then
				...
				__info_debug_message "$MSG" "$FUNC" "$UTIL_WORD_TO_TXT"
				for A in ${DOCFILES}
				do
					if [ -f "${A}" ]; then
				...
				__info_debug_message_end "Done" "$FUNC" "$UTIL_WORD_TO_TXT"
				return $SUCCESS
			fi
			MSG="Force exit!"
			__info_debug_message_end "$MSG" "$FUNC" "$UTIL_WORD_TO_TXT"
			return $NOT_SUCCESS
		fi
		MSG="Force exit!"
		__info_debug_message_end "$MSG" "$FUNC" "$UTIL_WORD_TO_TXT"

...
function __rm_leads() {
	...
	if [ -n "${FILES}" ]; then
		...
		__info_debug_message "$MSG" "$FUNC" "$UTIL_RM_LEADS"
		...
		do
			MSG="Checking file [${A}]?"
			__info_debug_message_que "$MSG" "$FUNC" "$UTIL_RM_LEADS"
			if [ -f "${A}" ]; then
				MSG="[ok]"
				__info_debug_message_ans "$MSG" "$FUNC" "$UTIL_RM_LEADS"
				file "${A}" | grep -q text
				STATUS=$?
				if [ $STATUS -eq $SUCCESS ]; then
		...
		__info_debug_message_end "Done" "$FUNC" "$UTIL_RM_LEADS"
		return $SUCCESS
	fi
	__usage RM_LEADS_USAGE
	return $NOT_SUCCESS
}

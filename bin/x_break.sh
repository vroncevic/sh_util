...
function __x_break() {
	...
	if [ -n "${TIME}" ]; then
		...
		__load_util_conf "$UTIL_X_BREAK_CFG" config_x_break
		STATUS=$?
		if [ $STATUS -eq $SUCCESS ]; then
				...
				+([0-9]))
					...
					do
						...
						if [ $STATUS -eq $SUCCESS ]; then
							...
							MSG="Time's up! Session will be closed!"
							__info_debug_message "$MSG" "$FUNC" "$UTIL_X_BREAK"
						fi
					done 
					__info_debug_message_end "Done" "$FUNC" "$UTIL_X_BREAK"
					return $SUCCESS
					;;
				*) 
					__usage X_BREAK_USAGE
					;;
			esac
			MSG="Force exit!"
			__info_debug_message_end "$MSG" "$FUNC" "$UTIL_X_BREAK"
			return $NOT_SUCCESS
		fi
		MSG="Force exit!"
		__info_debug_message_end "$MSG" "$FUNC" "$UTIL_X_BREAK"

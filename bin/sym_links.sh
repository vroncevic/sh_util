...
# fi
#
function __sym_links() {
	local DIR=$1
	if [ -n "${DIR}" ]; then
		local FUNC=${FUNCNAME[0]} MSG="None" LISTEDFILE
		MSG="Checking directory [${DIR}/]?"
		__info_debug_message_que "$MSG" "$FUNC" "$UTIL_SYM_LINKS"
		if [ -d "${DIR}/" ]; then
			MSG="[ok]"
			__info_debug_message_ans "$MSG" "$FUNC" "$UTIL_SYM_LINKS"
			MSG="Symbolic links in directory [${DIR}/]"
			__info_debug_message "$MSG" "$FUNC" "$UTIL_SYM_LINKS"
			...
			__info_debug_message_end "Done" "$FUNC" "$UTIL_SYM_LINKS"
			return $SUCCESS
		fi
		MSG="[not ok]"
		__info_debug_message_ans "$MSG" "$FUNC" "$UTIL_SYM_LINKS"
		...
		__info_debug_message "$MSG" "$FUNC" "$UTIL_SYM_LINKS"
		MSG="Force exit!"
		__info_debug_message_end "$MSG" "$FUNC" "$UTIL_SYM_LINKS"
		return $NOT_SUCCESS
	fi
	__usage SYM_LINKS_USAGE
	return $NOT_SUCCESS
}

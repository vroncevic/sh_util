...
# fi
#
function __strip_comment() {
	local FILE=$1
	if [ -n "${FILE}" ]; then
		local FUNC=${FUNCNAME[0]} MSG="None"
		MSG="Checking Code File [${FILE}]?"
		__info_debug_message_que "$MSG" "$FUNC" "$UTIL_STRIP_COMMENT"
		if [ -e "${FILE}" ]; then
			MSG="[ok]"
			__info_debug_message_ans "$MSG" "$FUNC" "$UTIL_STRIP_COMMENT"
			...
			if [ "$TYPE" != "${CORRECT_TYPE}" ]; then
				...
				__info_debug_message "$MSG" "$FUNC" "$UTIL_STRIP_COMMENT"
				MSG="Force exit!"
				__info_debug_message_end "$MSG" "$FUNC" "$UTIL_STRIP_COMMENT"
				return $NOT_SUCCESS
			fi
			MSG="[ok]"
			__info_debug_message_ans "$MSG" "$FUNC" "$UTIL_STRIP_COMMENT"
			sed '/^\/\*/d /.*\*\//d' ${FILE}
			__info_debug_message_end "Done" "$FUNC" "$UTIL_STRIP_COMMENT"
			return $SUCCESS
		fi
		MSG="[not ok]"
		__info_debug_message_ans "$MSG" "$FUNC" "$UTIL_STRIP_COMMENT"

...
function __blot_out() {
	...
	if [ -n "${FILE}" ]; then
		...
		__info_debug_message_que "$MSG" "$FUNC" "${UTIL_BLOT_OUT}"
		if [ ! -e "${FILE}" ]; then
			MSG="[not ok]"
			__info_debug_message_ans "$MSG" "$FUNC" "$UTIL_BLOT_OUT"
			MSG="Check file [${FILE}]!"
			__info_debug_message "$MSG" "$FUNC" "$UTIL_BLOT_OUT"
			MSG="Force exit!"
			__info_debug_message_end "$MSG" "$FUNC" "$UTIL_BLOT_OUT"
			return $NOT_SUCCESS
		fi
		MSG="[ok]"
		__info_debug_message_ans "$MSG" "$FUNC" "$UTIL_BLOT_OUT"
		...
		__info_debug_message "$MSG" "$FUNC" "$UTIL_BLOT_OUT"
		__info_debug_message_end "Done" "$FUNC" "$UTIL_BLOT_OUT"
		return $SUCCESS
	fi
	__usage BLOT_OUT_USAGE
	return $NOT_SUCCESS
}

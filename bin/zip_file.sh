	...
	if [ -n "${FILES}" ]; then
		...
		do
			case ${OPTIONS} in
				...
				\?) 
					MSG="Wrong argument!"
					__info_debug_message "$MSG" "$FUNC" "$UTIL_ZIP_FILE"
					MSG="Force exit!"
					__info_debug_message_end "$MSG" "$FUNC" "$UTIL_ZIP_FILE"
					return $NOT_SUCCESS
					;;
			esac
		done
		...
		__info_debug_message_end "Done" "$FUNC" "$UTIL_ZIP_FILE"
		return $SUCCESS
	fi
	__usage ZIP_FILE_USAGE
	return $NOT_SUCCESS
}

...
# fi
#
function __avi_to_mp4() {
	local FILE=$1
	if [ -n "${FILE}" ]; then
		local FUNC=${FUNCNAME[0]} MSG="None" STATUS
		declare -A config_avi_to_mp4=()
		__load_util_conf "$UTIL_AVI_TO_MP4_CFG" config_avi_to_mp4
		STATUS=$?
		MSG="Checking file [${FILE}]?"
		__info_debug_message_que "$MSG" "$FUNC" "$UTIL_AVI_TO_MP4"
		if [ -e "${FILE}" ]; then
			MSG="[ok]"
			__info_debug_message_ans "$MSG" "$FUNC" "$UTIL_AVI_TO_MP4"
			local FFMPEG=${config_avi_to_mp4[FFMPEG]}
			__check_tool "${FFMPEG}"
			STATUS=$?
			if [ $STATUS -eq $SUCCESS ]; then
				MSG="Converting [${FILE}] to MP4 format!"
				__info_debug_message "$MSG" "$FUNC" "$UTIL_AVI_TO_MP4"
				...
				__info_debug_message_end "Done" "$FUNC" "$UTIL_AVI_TO_MP4"
				return $SUCCESS
			fi
			MSG="Force exit!"
			__info_debug_message_end "$MSG" "$FUNC" "$UTIL_AVI_TO_MP4"
			return $NOT_SUCCESS
		fi
		MSG="[not ok]"
		__info_debug_message_ans "$MSG" "$FUNC" "$UTIL_AVI_TO_MP4"

...
# fi
#
function __java_heap_dump() {
	local PIDJVM=$1
	if [ -n "${PIDJVM}" ]; then
		local FUNC=${FUNCNAME[0]} MSG="None" STATUS
		declare -A config_java_heap_dump=()
		__load_util_conf "$UTIL_JAVA_HEAP_DUMP_CFG" config_java_heap_dump
		STATUS=$?
		if [ $STATUS -eq $SUCCESS ]; then
			...
			if [ $STATUS -eq $SUCCESS ]; then
				...
				__info_debug_message "$MSG" "$FUNC" "$UTIL_JAVA_HEAP_DUMP"
				...
				__info_debug_message_end "Done" "$FUNC" "$UTIL_JAVA_HEAP_DUMP"
				return $SUCCESS
			fi
		MSG="Force exit!"
		__info_debug_message_end "$MSG" "$FUNC" "$UTIL_JAVA_HEAP_DUMP"
		return $NOT_SUCCESS
	fi
	__usage JAVA_HEAP_DUMP_USAGE
	return $NOT_SUCCESS
}

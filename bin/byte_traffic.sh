...
# @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
#
# local BYTE_COUNT=$(__byte_count $INTER $OPTION)
#
function __byte_count() {
	...
	if [[ -n "${INTER}" && -n "${DIREC}" ]]; then
		...
		__info_debug_message "$MSG" "$FUNC" "$UTIL_BYTE_TRAFFIC"
		while read LINE
		do
			if [[ $LINE == $INTER:* ]]; then
	...
	MSG="Force exit!"
	__info_debug_message_end "$MSG" "$FUNC" "$UTIL_BYTE_TRAFFIC"
}
...
function __interface_check() {
	...
	if [ -n "${INTER}" ]; then
		...
		__info_debug_message_end "Done" "$FUNC" "$UTIL_BYTE_TRAFFIC"
		return $SUCCESS
	fi
	MSG="Force exit!"
	__info_debug_message_end "$MSG" "$FUNC" "$UTIL_BYTE_TRAFFIC"
		...
		case "${TIME}" in
			^[0-9]+[smhd]$)
				...
				if [ $STATUS -eq $SUCCESS ]; then
					...
					[[ ${UNIT} == h ]] && DIV=3600
					[[ ${UNIT} == d ]] && DIV=86400
					local RECEIVED TRANSMIT NRECEIVED NTRANSMIT RDIFF TDIFF RATE
					while grep "${INTER}" < /proc/net/dev &>/dev/null
					do
						RECEIVED=$(__byte_count "${INTER}" r)
						TRANSMIT=$(__byte_count "${INTER}" t)
						sleep ${TIME}
						NRECEIVED=$(__byte_count "${INTER}" r)
						NTRANSMIT=$(__byte_count "${INTER}" t)

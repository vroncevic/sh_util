...
# @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
#
# local TXT=$(__get_txt $ERRCODE $LISTQUERY)
#
function __get_txt() {
...
# @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
#
# __check_address "$REVDNS" "$LISTSERVER"
# local STATUS=$?
#
...
# fi
#
function __is_spammer(){
	local DOMAIN=$1
	if [ -n "${DOMAIN}" ]; then
		local FUNC=${FUNCNAME[0]} MSG="None"
		MSG="Get address of [${DOMAIN}]!"
		__info_debug_message "$MSG" "$FUNC" "$UTIL_IS_SPAMMER"
		...
		if [ ${#IPADR} -gt 6 ]; then
			...
			printf "%s" " $(__check_address ${R_DNS} 'sbl-xbl.spamhaus.org')"
			printf "%s" " ordb.org  says "
			printf "%s" " $(__check_address ${R_DNS} 'relays.ordb.org')"
			printf "%s" " spamcop.net says "
			printf "%s" " $(__check_address ${R_DNS} 'bl.spamcop.net')"
			printf "%s" " abuseat.org says "
			printf "%s" " $(__check_address ${R_DNS} 'cbl.abuseat.org')"
			printf "%s" " Distributed Server Listings"
			printf "%s" " list.dsbl.org says "
			printf "%s" " $(__check_address ${R_DNS} 'list.dsbl.org')"
			printf "%s" " multihop.dsbl.org says "
			printf "%s" " $(__check_address ${R_DNS} 'multihop.dsbl.org')"
			printf "%s" " unconfirmed.dsbl.org says "
			printf "%s" " $(__check_address ${R_DNS} 'unconfirmed.dsbl.org')"
			__info_debug_message_end "Done" "$FUNC" "$UTIL_IS_SPAMMER"
			return $SUCCESS
		fi
		MSG="Could not use that address!"
		__info_debug_message "$MSG" "$FUNC" "$UTIL_IS_SPAMMER"
		MSG="Force exit!"
		__info_debug_message_end "$MSG" "$FUNC" "$UTIL_IS_SPAMMER"
		return $NOT_SUCCESS
	fi
	__usage IS_SPAMMER_USAGE
	return $NOT_SUCCESS
}

...
function __longer_lines() {
	...
	if [[ -n "${NUMCHARS}" && -n "${FILES}" ]]; then
		...
		__info_debug_message "$MSG" "$FUNC" "$UTIL_LONGER_LINES"
		case ${NUMCHARS} in
			+([0-9]))
				for A in "${FILES[@]}"
				do
			...
			*)
				...
				__info_debug_message "$MSG" "$FUNC" "$UTIL_LONGER_LINES"
				MSG="Force exit!"
				__info_debug_message_end "$MSG" "$FUNC" "$UTIL_LONGER_LINES"
				return $NOT_SUCCESS
				;;
		esac
		__info_debug_message_end "Done" "$FUNC" "$UTIL_LONGER_LINES"
		return $SUCCESS
	fi
	__usage LONGER_LINES_USAGE
	return $NOT_SUCCESS
}

...
function __logged_out() {
	...
	if [[ -n "${USR}" && -n "${TIME}" ]]; then
		...
		__info_debug_message "$MSG" "$FUNC" "$UTIL_LOGGED_OUT"
		who | grep "^${USR} " 2>&1 > /dev/null
		if [[ $? != 0 ]]; then
			MSG="User [${USR}] is not logged in!"
			__info_debug_message "$MSG" "$FUNC" "$UTIL_LOGGED_OUT"
			__info_debug_message_end "Done" "$FUNC" "$UTIL_LOGGED_OUT"
			return $SUCCESS
		fi

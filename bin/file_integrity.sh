...
function __setup_db() {
	...
	if [[ -n "${FILE}" && -n "${DIR}" ]]; then
		...
		__load_util_conf "$UTIL_FILE_INTEGRITY_CFG" config_file_integrity
		STATUS=$?
		if [ $STATUS -eq $SUCCESS ]; then
			local MD5SUM=${config_file_integrity[MD5SUM]}
			if [ -e "${FILE}" ];
				MSG="Write directory name to first line of file!"
				__info_debug_message "$MSG" "$FUNC" "$UTIL_FILE_INTEGRITY"
				echo "${DIR}" > "${FILE}"
				MSG="Append md5 checksums and filenames!"
				__info_debug_message "$MSG" "$FUNC" "$UTIL_FILE_INTEGRITY"
				eval "${MD5SUM} ${DIR}/* >> ${FILE}"
				__info_debug_message_end "Done" "$FUNC" "$UTIL_FILE_INTEGRITY"
				return $SUCCESS
			fi
			MSG="Please check file [${FILE}]!"
			__info_debug_message "$MSG" "$FUNC" "$UTIL_FILE_INTEGRITY"
			MSG="Force exit!"
			__info_debug_message_end "$MSG" "$FUNC" "$UTIL_FILE_INTEGRITY"
			return $NOT_SUCCESS
		fi
		MSG="Force exit!"
		__info_debug_message_end "$MSG" "$FUNC" "$UTIL_FILE_INTEGRITY"
...
function __check_db() {
	...
	if [ -n "${FILE}" ]; then
		...
		__info_debug_message_que "$MSG" "$FUNC" "$UTIL_FILE_INTEGRITY"
		if [ ! -r "${FILE}" ]; then
			MSG="[not ok]"
			__info_debug_message_ans "$MSG" "$FUNC" "$UTIL_FILE_INTEGRITY"

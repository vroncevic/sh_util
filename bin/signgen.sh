...
function __signgen() {
    ...
    if [ -n "$NAME" ] && [ -n "$WORKPOSITION" ] && [ -n "$DEPARTMENT" ] &&
       [ -n "$IP" ] && [ -n "$MOBILE" ] && [ -n "$EMAIL" ] ; then
		...
		local MSG=""
		declare -A cfgsigngen=()
		__loadutilconf $UTIL_CFG_SIGNGEN cfgsigngen
		local STATUS=$?
		if [ "$STATUS" -eq "$SUCCESS" ]; then
			if [ "$TOOL_DBG" == "true" ]; then
				MSG="Checking dir [$cfgsigngen[COMPANY_EMPLOYEE]/]"
...
<html>
    ...
    <head>
        <title>$cfgsigngen[COMPANY_NAME]</title>
        <style>
            table-params {
                border: 0;
                border-spacing:2px;

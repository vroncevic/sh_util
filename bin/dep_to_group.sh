...
# fi
#
function __dep_to_group() {
	local DEP=$1
	if [ -n "${DEP}" ]; then
		local FUNC=${FUNCNAME[0]} MSG="None" STATUS
		declare -A config_dep_to_group=()
		__load_util_conf "$UTIL_DEP_TO_GROUP_CFG" config_dep_to_group
		STATUS=$?
		if [ $STATUS -eq $SUCCESS ]; then
			MSG="Convert name of department to group!"
			__info_debug_message "$MSG" "$FUNC" "$UTIL_DEP_TO_GROUP"
			...
			__info_debug_message_end "Done" "$FUNC" "$UTIL_DEP_TO_GROUP"
			return $SUCCESS
		fi
		MSG="Force exit!"
		__info_debug_message_end "$MSG" "$FUNC" "$UTIL_DEP_TO_GROUP"
		return $NOT_SUCCESS
	fi
	__usage DEP_TO_GROUP_USAGE
	return $NOT_SUCCESS
}

...
function __rm_lines() {
	...
	if [[ -n "${IFILE}" && -n "${OFILE}" ]]; then
		...
		__info_debug_message_que "$MSG" "$FUNC" "$UTIL_RM_LINES"
		if [ -f "${IFILE}" ]; then
			MSG="[ok]"
			__info_debug_message_ans "$MSG" "$FUNC" "$UTIL_RM_LINES"
			local TMP1=/tmp/TMP.${RANDOM}$$
			trap 'rm -f $TMP1 >/dev/null 2>&1' 0
			trap "exit 1" 1 2 3 15
			sed -e 's/ //g' -e 's-^-/-g' -e 's-$-/d-' ${IFILE} > ${TMP1}
			sed -f "${TMP1}" "${OFILE}"
			__info_debug_message_end "Done" "$FUNC" "$UTIL_RM_LINES"
			return $SUCCESS
		fi
		MSG="[not ok]"
		__info_debug_message_ans "$MSG" "$FUNC" "$UTIL_RM_LINES"
		MSG="Please check file [${IFILE}]!"
		__info_debug_message "$MSG" "$FUNC" "$UTIL_RM_LINES"
		MSG="Force exit!"
		__info_debug_message_end "$MSG" "$FUNC" "$UTIL_RM_LINES"
		return $NOT_SUCCESS
	fi
	__usage RM_LINES_USAGE
	return $NOT_SUCCESS
}

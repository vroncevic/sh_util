...
# @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
#
# __ssh_config "vroncevic" "vroncevic"
# local STATUS=$?
#
...
# fi
#
function __ssh_config() {
	local USR=$1 DEP=$2
	if [[ -n "${USR}" && -n "${DEP}" ]]; then
		local FUNC=${FUNCNAME[0]} MSG="None" STATUS
		declare -A config_ssh_config=()
		__load_util_conf "$UTIL_SSH_CONFIG_CFG" config_ssh_config
		STATUS=$?
		if [ $STATUS -eq $SUCCESS ]; then
			MSG="Generating SSH client config file at ${USR} home!"
			__info_debug_message "$MSG" "$FUNC" "$UTIL_SSH_CONFIG"
			local UHOME="/home/${USR}"
			MSG="Checking user home directory [${UHOME}/]?"
			if [ -d "${UHOME}/" ]; then
				MSG="[ok]"
				__info_debug_message_ans "$MSG" "$FUNC" "$UTIL_SSH_CONFIG"
				MSG="Checking directory [${UHOME}/.ssh/]?"
				__info_debug_message_que "$MSG" "$FUNC" "$UTIL_SSH_CONFIG"
				if [ ! -d "${UHOME}/.ssh/" ]; then
					MSG="[not ok]"
					__info_debug_message_ans "$MSG" "$FUNC" "$UTIL_SSH_CONFIG"
				...
				__info_debug_message_end "Done" "$FUNC" "$UTIL_SSH_CONFIG"
				return $SUCCESS
			fi
			MSG="[not ok]"
			__info_debug_message_ans "$MSG" "$FUNC" "$UTIL_SSH_CONFIG"
			...
			__info_debug_message "$MSG" "$FUNC" "$UTIL_SSH_CONFIG"
			MSG="Force exit!"
			__info_debug_message_end "$MSG" "$FUNC" "$UTIL_SSH_CONFIG"
			return $NOT_SUCCESS
		fi
		MSG="Force exit!"
		__info_debug_message_end "$MSG" "$FUNC" "$UTIL_SSH_CONFIG"

...
function __low_swap() {
	...
	if [[ -n "${SWAPLIMIT}" && -n "${EMAIL}" ]]; then
		...
		__info_debug_message "$MSG" "$FUNC" "$UTIL_LOW_SWAP"
		case ${SWAPLIMIT} in
			+([0-9]))
				SWAP_FREE=$(free -mo | grep Swap | { read a b c d; echo $d; })
				if [[ ${SWAP_FREE} < ${SWAPLIMIT} ]]; then
					MSG="Swap is running low! Less then ${SWAPLIMIT} MB!"
					__send_mail "$MSG" "$EMAIL"
					STATUS=$?
					if [ $STATUS -eq $NOT_SUCCESS ]; then
						MSG="Force exit!"
						__info_debug_message_end "$MSG" "$FUNC" "$UTIL_LOW_SWAP"
						return $NOT_SUCCESS
					fi
					__info_debug_message_end "Done" "$FUNC" "$UTIL_LOW_SWAP"
					return $SUCCESS
				fi
				MSG="Swap memory ok!"
				__info_debug_message "$MSG" "$FUNC" "$UTIL_LOW_SWAP"

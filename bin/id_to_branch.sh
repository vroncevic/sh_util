...
# fi
#
function __id_to_branch() {
	local ID=$1
	if [ -n "${ID}" ]; then
		local FUNC=${FUNCNAME[0]} MSG="None" STATUS K
		declare -A config_id_to_branch=()
		__load_util_conf "$UTIL_ID_TO_BRANCH_CFG" config_id_to_branch
		STATUS=$?
		if [ $STATUS -eq $SUCCESS ]; then
			MSG="Converting [${ID}] to branch name!"
			__info_debug_message "$MSG" "$FUNC" "$UTIL_ID_TO_BRANCH"
			for K in "${!config_id_to_branch[@]}"
			do
				if [ "${K}" == "${ID}" ]; then
					eval "$2=$(printf "'%s' " "${config_id_to_branch[${K}]}")"
					__info_debug_message_end "Done" "$FUNC" "$UTIL_ID_TO_BRANCH"
					return $SUCCESS
				fi
			done
			...
			__info_debug_message "$MSG" "$FUNC" "$UTIL_ID_TO_BRANCH"
			MSG="Force exit"
			__info_debug_message_end "$MSG" "$FUNC" "$UTIL_ID_TO_BRANCH"
			return $NOT_SUCCESS
		fi
		MSG="Force exit"
		__info_debug_message_end "$MSG" "$FUNC" "$UTIL_ID_TO_BRANCH"

...
# fi
#
function __vbox_config() {
	local USR=$1
	if [ -n "${USR}" ]; then
		local FUNC=${FUNCNAME[0]} MSG="None" STATUS
		declare -A config_vbox_config=()
		__load_util_conf "$UTIL_CFG_VBOXCFG" config_vbox_config
		STATUS=$?
		if [ $STATUS -eq $SUCCESS ]; then
			local VBOXCONF=${config_vbox_config[VBOXCONF]}
			MSG="Checking directory [${VBOXCONF}/]?"
			__info_debug_message_que "$MSG" "$FUNC" "$UTIL_VBOX_CONFIG"
			if [ -d "${VBOXCONF}/" ]; then
				MSG="[ok]"
				__info_debug_message_ans "$MSG" "$FUNC" "$UTIL_VBOX_CONFIG"
				...
				if [ ! -d "$VBOXUSR/" ]; then
					...
					__info_debug_message "$MSG" "$FUNC" "$UTIL_VBOX_CONFIG"
					mkdir "${VBOXUSR}/"
				else
					MSG="[ok]"
					__info_debug_message_ans "$MSG" "$FUNC" "$UTIL_VBOX_CONFIG"
				...
				__info_debug_message_end "Done" "$FUNC" "$UTIL_VBOX_CONFIG"
				return $SUCCESS
			fi
			MSG="[not ok]"
			__info_debug_message_ans "$MSG" "$FUNC" "$UTIL_VBOX_CONFIG"
			...
			__info_debug_message "$MSG" "$FUNC" "$UTIL_VBOX_CONFIG"
			MSG="Force exit!"
			__info_debug_message_end "$MSG" "$FUNC" "$UTIL_VBOX_CONFIG"
			return $NOT_SUCCESS
		fi
		MSG="Force exit!"
		__info_debug_message_end "$MSG" "$FUNC" "$UTIL_VBOX_CONFIG"

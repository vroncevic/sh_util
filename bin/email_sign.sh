...
function __email_sign() {
	...
	if [ ${#SIGN_STRUCT[@]} -eq 6 ] ; then
		...
		if [ $STATUS -eq $NOT_SUCCESS ]; then
			__usage EMAIL_SIGN_USAGE
			return $NOT_SUCCESS
		fi
		declare -A config_email_sign=()
		__load_util_conf $UTIL_CFG_ESIGNATURE config_email_sign
		STATUS=$?
		if [ $STATUS -eq $SUCCESS ]; then
			local CDIR=${config_email_sign[COMPANY_EMPLOYEES]} SLINE
			MSG="Checking directory [${CDIR}/]?"
			__info_debug_message_que "$MSG" "$FUNC" "$UTIL_EMAIl_SIGN"
			if [ -d "${CDIR}/" ]; then
				MSG="[ok]"
				__info_debug_message_ans "$MSG" "$FUNC" "$UTIL_EMAIl_SIGN"
				...
				MSG="Set permission!"
				__info_debug_message "$MSG" "$FUNC" "$UTIL_EMAIl_SIGN"
				eval "chmod 644 ${CDIR}/${NAME}"
				__info_debug_message_end "Done" "$FUNC" "$UTIL_EMAIl_SIGN"
				return $SUCCESS
			fi
			MSG="[not ok]"
			__info_debug_message_ans "$MSG" "$FUNC" "$UTIL_EMAIl_SIGN"
			...
			__info_debug_message "$MSG" "$FUNC" "$UTIL_EMAIl_SIGN"
			MSG="Force exit!"
			__info_debug_message_end "$MSG" "$FUNC" "$UTIL_EMAIl_SIGN"
			return $NOT_SUCCESS
		fi
		MSG="Force exit!"
		__info_debug_message_end "$MSG" "$FUNC" "$UTIL_EMAIl_SIGN"

...
# fi
#
function __unpack_to_DIR() {
	shift $((${OPTIND} - 1))
	local STARTDIR=$(pwd) FUNC=${FUNCNAME[0]} MSG="None" A DIR
	MSG="Unpack archive to DIR archive!"
	__info_debug_message "$MSG" "$FUNC" "$UTIL_UNPACK_TO_DIR"
	for A in "$@"
	do
		cd ${STARTDIR}
		local FNAME=$(__getbasename ${A}) DIR=$(__getdirname ${A})
		cd ${DIR}
		A=${FNAME}
		case ${A} in
			# zip
			...
			*)
				...
				__info_debug_message "$MSG" "$FUNC" "$UTIL_UNPACK_TO_DIR"
				MSG="Force exit!"
				__info_debug_message_end "$MSG" "$FUNC" "$UTIL_UNPACK_TO_DIR"
				return $NOT_SUCCESS
				;;
		esac
	done
	__info_debug_message_end "Done" "$FUNC" "$UTIL_UNPACK_TO_DIR"
	return $SUCCESS
}

...
#
if [ -z "$__SH_UTIL_DISK_LABEL" ]; then
    readonly __SH_UTIL_DISK_LABEL=1
    ...
    function disk_label {
        ...
        if [[ -z "${DRIVE}" || -z "${LABEL}" ]]; then
            usage LABEL_USAGE
            return $NOT_SUCCESS
        fi
        local FUNC=${FUNCNAME[0]} MSG="None" STATUS
        declare -A config_disk_label=()
        load_util_conf "$UTIL_LABEL_CFG" config_disk_label
        STATUS=$?
        if [ $STATUS -eq $SUCCESS ]; then
            ...
            if [ $STATUS -eq $SUCCESS ]; then
                ...
                info_debug_message_que "$MSG" "$FUNC" "$UTIL_LABEL"
                local MNTCHK=$(mount | grep -c /mnt/$LABEL)
                local MNTGRP=$(grep -qs /mnt/${LABEL} /proc/mounts)
                if [ ${MNTCHK} != 0 ] && [ ${MNTGRP} ]; then
                    MSG="[ok]"
                    info_debug_message_ans "$MSG" "$FUNC" "$UTIL_LABEL"
                    MSG="Set label!"
                    info_debug_message "$MSG" "$FUNC" "$UTIL_LABEL"
                    eval "${MLAB} -i /dev/${DRIVE} ::${LABEL_NAME}"
                    info_debug_message_end "Done" "$FUNC" "$UTIL_LABEL"
                    return $SUCCESS
                fi
                MSG="[not ok]"
                info_debug_message_ans "$MSG" "$FUNC" "$UTIL_LABEL"

...
#
if [ -z "$__SH_UTIL_GZ_TO_BZ2" ]; then
    readonly __SH_UTIL_GZ_TO_BZ2=1
    ...
    function gz_to_bz2 {
        ...
        info_debug_message "$MSG" "$FUNC" "$UTIL_GZ_TO_BZ2"
        if [ -e "${FILE}" ]; then
            local STATUS PV GZ BZ
            declare -A config_gz_to_bz2=()
            load_util_conf "$UTIL_GZ_TO_BZ2_CFG" config_gz_to_bz2
            STATUS=$? 
            if [ $STATUS -eq $SUCCESS ]; then
                ...
                if [ $STATUS -eq $SUCCESS ]; then
                    ...
                    eval "time ${GZ} | ${PV} | ${BZ}"
                    info_debug_message_end "Done" "$FUNC" "$UTIL_GZ_TO_BZ2"
                    return $SUCCESS
                fi
                MSG="Force exit!"
